...
# Version: Alpha v0.0005-FINAL | Clean Merged main.py
# =============================================================================
...
# =============================================================================
# BLOCK 3: Core GUI Dropdown Utility Imports & Display Modules
# =============================================================================
...
# =============================================================================
# BLOCK 4: CS2ParserApp Class — Main GUI Application
# =============================================================================
...
class CS2ParserApp:
    ...
    def _tabs(self):
        ...
        # Optional placeholder text for Scout Report tab
        ttk.Label(
    ...
    def extract_players_and_rounds(self, events):
        """
        Extract unique players and rounds from event list for dropdowns.

        Args:
            events (list): List of event dicts parsed from demo.

        Returns:
            tuple: (players_list, rounds_list)
                players_list: List of strings like "PlayerName (SteamID)"
                rounds_list: List of strings like "Round 1", "Round 2", etc.
        """
        players_set = set()
        rounds_set = set()
    ...
    def run_parse(self):
        ...
        # Launch threaded parse safely with class method
    ...
    # FILE SELECTION AND TAB POPULATION
    ...
    def select_file(self):
        ...
        try:
            ...
            # Inject dropdown values from event log for reliability
    ...
    def _populate_tabs(self):
        try:
            ...
            # Trigger dropdown refresh as fallback
            try:
    ...
    def _inject_dropdown_from_event_log(self):
        """
        Extract players and rounds from event log to populate dropdowns reliably.
        """
        from utils.steam_utils import to_steam2
    ...
    # DISPLAY AND DEBUG CONTROLS
    # ---------------------------
    # This section manages the UI elements responsible for displaying parsed data,
    # debugging information, and controls for interacting with the loaded data.

    def setup_display_controls(parent_frame):
        """
        Setup UI widgets such as Treeviews, text consoles, dropdowns,
        and buttons used for displaying parsed demo data and debug logs.
        """
        # Example: Setup Treeview for event logs
        event_tree = ttk.Treeview(parent_frame)
        event_tree.pack(fill='both', expand=True)

        # Example: Setup console output for debug messages
        debug_console = ScrolledText(parent_frame, height=10, state='disabled')
        debug_console.pack(fill='x')

        # Example: Setup buttons for toggling debug info or refreshing display
        btn_refresh = ttk.Button(parent_frame, text="Refresh", command=refresh_display)
        btn_refresh.pack(side='left')

        btn_toggle_debug = ttk.Button(parent_frame, text="Toggle Debug", command=toggle_debug)
        btn_toggle_debug.pack(side='left')

        # Store or return widgets as needed

    def refresh_display():
        """Refresh displayed data based on current selections."""
        pass

    def toggle_debug():
        """Enable or disable debug logging display."""
        pass
