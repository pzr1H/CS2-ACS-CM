...
def event_log_tab_controller(tab_frame, data):
    """Working event log tab controller"""
    try:
        import tkinter as tk
        ...
        # Title
        title_label = tk.Label(main_frame, text="Event Log", 
        ...
        for i, event in enumerate(events[:500]):  # Limit to 500 events
            if isinstance(event, dict):
                event_type = event.get("type", "unknown")
                time_val = event.get("tick", i)
                details = f"Tick: {time_val}"
                
                tree.insert("", "end", values=(time_val, event_type, details))
        
        log.info(f"✅ Event log populated with {len(events)} events")
        
    except Exception as e:
        import tkinter as tk
        log.error(f"Event log error: {e}")
        error_label = tk.Label(tab_frame, text=f"Event Log Error: {str(e)}", 
                             fg="red", bg="black")
